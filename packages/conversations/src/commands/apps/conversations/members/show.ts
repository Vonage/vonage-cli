...
> {
    ...
    async run() {
        ...
        const response = await this.getMemberById({ ...args, ...flags });

        this.log(chalk.magenta.underline.bold('Member ID:'), response.data.id);
        this.log('');
        this.log(
            chalk.magenta.underline.bold('User ID:'),
            response.data._embedded.user.id,
        );
        this.log('');
        this.log(
            chalk.magenta.underline.bold('User Name:'),
            response.data._embedded.user.name,
        );
        this.log('');
