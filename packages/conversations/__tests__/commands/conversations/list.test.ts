...
describe('apps:conversations', () => {
    test.stdout()
        .nock(baseHost, (api) =>
            api.get('/conversations').reply(200, listResponse),
        )
    ...
    test.stdout()
        .nock(baseHost, (api) =>
            api.get('/conversations').reply(400, genericFailedError),
        )
        .command(['apps:conversations'])
        .catch((ctx) => {
    ...
    test.stdout()
        .nock(baseHost, (api) =>
            api.get('/conversations').reply(401, genericAuthError),
        )
        .command(['apps:conversations'])
        .catch((ctx) => {
    ...
    test.stdout()
        .nock(baseHost, (api) =>
            api.get('/conversations').reply(500, internalError),
        )
        .command(['apps:conversations'])
        .catch((ctx) => {
