...
describe('apps:conversations:update', () => {
    test.stdout()
        .nock(baseHost, (api) =>
            api
                .put(
                    '/conversations/CON-d66d47de-5bcb-4300-94f0-0c9d4b948e9a',
                    { name: 'my_conversation' },
                )
                .reply(200, conversationResponse),
        )
        .command([
            'apps:conversations:update',
    ...
    test.stdout()
        .nock(baseHost, (api) =>
            api.put('/conversations/12345').reply(400, genericFailedError),
        )
    ...
    test.stdout()
        .nock(baseHost, (api) =>
            api.put('/conversations/12345').reply(401, genericAuthError),
        )
        .command(['apps:conversations:update', '12345'])
        .catch((ctx) => {
    ...
    test.stdout()
        .nock(baseHost, (api) =>
            api.put('/conversations/12345').reply(404, notFoundError),
        )
        .command(['apps:conversations:update', '12345'])
        .catch((ctx) => {
    ...
    test.stdout()
        .nock(baseHost, (api) =>
            api.put('/conversations/12345').reply(500, internalError),
        )
        .command(['apps:conversations:update', '12345'])
        .catch((ctx) => {
