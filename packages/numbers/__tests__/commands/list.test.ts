...
describe('numbers:list', async () => {
    const baseHost = 'https://rest.nexmo.com/account/numbers';

    test.nock(baseHost, (api) =>
        api.get('').query(keysOnlyBody).reply(200, numbersResponse),
    )
        .stdout()
        .command(['numbers'])
    ...
    test.nock(baseHost, (api) =>
        api.get('').query(keysOnlyBody).reply(420, genericFailedError),
    )
        .command(['numbers'])
        .catch((ctx) => {
    ...
    test.nock(baseHost, (api) =>
        api.get('').query(keysOnlyBody).reply(401, genericAuthError),
    )
        .command(['numbers'])
        .catch((ctx) => {
