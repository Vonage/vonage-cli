...
describe('numbers:search', async () => {
    test.stdout()
        .nock(baseHost, (api) =>
            api.get('/search').query(searchBody).reply(200, searchResponse),
        )
    ...
    test.stdout()
        .nock(baseHost, (api) =>
            api.get('/search').query(searchBody).reply(420, genericFailedError),
        )
        .command(['numbers:search', 'GB'])
        .catch((ctx) => {
    ...
    test.stdout()
        .nock(baseHost, (api) =>
            api.get('/search').query(searchBody).reply(401, genericAuthError),
        )
        .command(['numbers:search', 'GB'])
        .catch((ctx) => {
