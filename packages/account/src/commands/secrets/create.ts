...
`,
    ...
    async validateSecret(secret: string) {
        ...
        return (
            regexCharacters.test(secret) &&
            regexLower.test(secret) &&
            regexUpper.test(secret) &&
            regexDigit.test(secret)
        )
    }
      ...
      valid = await this.validateSecret(secret)
        ...
        if (valid) {
            ...
            if (response.status === '201') {
                this.log(
                    `${response.status}: Success id: ${response.id}  created at: ${response.created_at}`
                )
            } else {
                this.log(
                    `${response.status}: ${response.title} - ${response.detail}`
                )
            }
        } else {
