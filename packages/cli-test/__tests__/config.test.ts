...
const getMockFS = (): FSFactory => ({
  loadFile: jest.fn(),
  saveFile: jest.fn(),
} as unknown as FSFactory);
...
describe('Config tests', () => {
  ...
  test('Will Load all config values', () => {
    ...
    // global config first
    (loadFile as jest.MockedFunction<LoadFileCurry>).mockReturnValueOnce(
      JSON.stringify({
  ...
  test('Will use local when global not found', () => {
    ...
    (loadFile as jest.MockedFunction<LoadFileCurry>).mockReturnValueOnce(null).mockReturnValue(
      JSON.stringify({
        [ConfigParams.API_KEY]: 'local-api-key',
        [ConfigParams.API_SECRET]: 'local-api-secret',
        [ConfigParams.PRIVATE_KEY]: 'local-private-key',
  ...
  test('Will null config as default', () => {
    ...
    (loadFile as jest.MockedFunction<LoadFileCurry>).mockReturnValue(null);
