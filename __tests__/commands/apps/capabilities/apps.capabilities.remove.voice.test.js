...
suite('Command: vonage apps capabilities rm voice', () => {
  beforeEach(() => {
    mockConsole();
  ...
  test('Will remove voice', async () => runRemoveCapabilityTest({
    handler,
    testCase: voiceDataSets[11],
    exitMock,
    confirmMock,
    confirmed: true,
  }));

  test('Will not remove voice when user declines', async () => runRemoveCapabilityTest({
    handler,
    testCase: voiceDataSets[11],
    exitMock,
    confirmMock,
