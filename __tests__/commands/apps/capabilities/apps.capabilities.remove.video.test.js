...
suite('Command: vonage apps capabilities rm video', () => {
  beforeEach(() => {
    mockConsole();
  ...
  test('Will remove Video', async () => runRemoveCapabilityTest({
    handler,
    testCase: videoDataSets[72],
    exitMock,
    confirmMock,
    confirmed: true,
  }));

  test('Will not remove Video when user declines', async () => runRemoveCapabilityTest({
    handler,
    testCase: videoDataSets[72],
    exitMock,
    confirmMock,
