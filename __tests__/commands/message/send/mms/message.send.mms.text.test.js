...
const { handler } = await loadModule(
  import.meta.url,
  '../../../../../src/commands/message/send/mms/text.js',
);

suite('Command: vonage message send mms text', { concurrency: 1 }, () => {
  beforeEach(() => {
    mockConsole();
  });

  test('Will send an MMS text message', async () => {
    ...
    assertSentMessage(sendMock, MMSText, {
      to: argv.to,
      from: argv.from,
      text: argv.text,
