...
const { handler } = await loadModule(import.meta.url, '../../../../../src/commands/message/send/rcs/video.js');

suite('Command: vonage message send rcs video', { concurrency: 1 }, () => {
  beforeEach(() => {
    mockConsole();
  });

  test('Will send an RCS video message', async () => {
    ...
    assertSentMessage(sendMock, RCSVideo, {
      to: argv.to,
      from: argv.from,
      video: { url: argv.url },
      ttl: argv.ttl,
  ...
  test('Will send an RCS video message without category', async () => {
    const messageUUID = faker.string.uuid();
    const { sendMock, sdkMock } = buildMessagesSDK(messageUUID);
    const argv = {
      SDK: sdkMock,
