...
const { handler } = await loadModule(import.meta.url, '../../../../../src/commands/message/send/whatsapp/file.js');

suite('Command: vonage message send whatsapp file', { concurrency: 1 }, () => {
  beforeEach(() => {
    mockConsole();
  });

  test('Will send a WhatsApp file message', async () => {
    ...
    assertSentMessage(sendMock, WhatsAppFile, {
      to: argv.to,
      from: argv.from,
      file: { url: argv.url },
